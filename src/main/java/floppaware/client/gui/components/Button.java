...
import floppaware.api.setting.Type;
...
public class Button implements Component
{
  ...
  public Button(Module mod, Frame parent, int offset)
  {
    ...
    height = 12;
    int opY = offset;
    if(Floppa.settings.getSettingsByMod(mod) != null)
    {
      for(Setting s : Floppa.settings.getSettingsByMod(mod))
      {
        if(s.type == Type.Mode)
        {
  ...
  public void renderComponent(MatrixStack m)
  {
    ...
    if(this.isHovered)
      MC.textRenderer.drawWithShadow(m,
              ...
              Floppa.MC.getWindow().getScaledWidth() - Floppa.MC.textRenderer.getWidth(this.mod.getDescription()) - 1,
              1, -1);
