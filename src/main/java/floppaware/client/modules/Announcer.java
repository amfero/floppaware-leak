...
public class Announcer extends Module
{
  ...
  onBlockPlace(PlaceBlockEvent event)
  {
    ...
    {
      if(placeAmount == 1)
        mc.player.sendChatMessage("I just placed a block of " + lastPlacedBlock.getName().getString() + "! | Floppware v1.0");
      else
        mc.player.sendChatMessage("I just placed " + (placeAmount == 1 ? 1 : placeAmount / 2) + " blocks of " + lastPlacedBlock.getName().getString() + "!| Floppware v1.0");
      placeDelay = 0;
      placeAmount = 0;
    }
  }
  ...
  onBlockBreak(BreakBlockEvent event)
  {
    ...
    {
      if(breakAmount == 1)
        mc.player.sendChatMessage("I just broke a block of " + lastBrokenBlock.getName().getString() + "!| Floppware v1.0");
      else
        mc.player.sendChatMessage("I just broke " + breakAmount + " blocks of " + lastBrokenBlock.getName().getString() + "!| Floppware v1.0");
      breakDelay = 0;
      breakAmount = 0;
    }
  }
  ...
  onDrop(DropItemEvent event)
  {
    ...
    {
      mc.player.sendChatMessage("I just dropped " + event.itemStack.getCount() + " " + event.itemStack.getItem().toString().replace("_", " ") + "!| Floppware v1.0");
      dropDelay = 0;
    }
  }
  ...
  onTick(TickEvent event)
  {
    ...
    if(walk.get() && walkDelay >= delayy)
    {
      ...
      {
        mc.player.sendChatMessage("I just walked " + new DecimalFormat("#").format(distance) + " blocks!| Floppware v1.0");
      }
    ...
    {
      mc.player.sendChatMessage("I just swapped hands, there is now a " + mc.player.getOffHandStack().getItem().toString().replace("_", " ") + " in my offhand!| Floppware v1.0");
      swapDelay = 0;
    }
    ...
    if(eat.get() && mc.options.keyUse.isPressed() && eatDelay >= delayy)
    {
      ...
      if(food.isFood())
      {
        mc.player.sendChatMessage("I am currently eating " + food.toString().replace("_", " ") + "!| Floppware v1.0");
        eatDelay = 0;
      }
    }
    ...
    {
      mc.player.sendChatMessage("I just jumped!| Floppware v1.0");
      jumpDelay = 0;
    }

    if(sneak.get() && mc.options.keySneak.isPressed() && swapDelay >= delayy)
    {
      mc.player.sendChatMessage("I just crouched!| Floppware v1.0");
      swapDelay = 0;
    }
    if(playerList.get() && mc.options.keyPlayerList.isPressed() && playerListDelay >= delayy)
    {
      mc.player.sendChatMessage("I just opened the playerlist!| Floppware v1.0");
      playerListDelay = 0;
    }
    ...
    if(experienceDelay >= delayy)
    {
      ...
      {
        mc.player.sendChatMessage("I just gained " + (currentExperience - lastExperience) + " experience!| Floppware v1.0");
      }
      experienceDelay = 0;
